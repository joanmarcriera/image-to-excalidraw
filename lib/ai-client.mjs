...
const BALANCED_RULES = [
  ...
  "- Avoid overlapping nodes or floating connectors.",
  "- Prefer short labels and a clean flow when the image allows it.",
].join("\n");

const STRUCTURED_RULES = [
  "- First plan the full layout before writing the JSON.",
  "- Use a grid-like layout with even spacing between nodes.",
  "- Prefer left-to-right flow unless the image clearly suggests another direction.",
  "- Avoid overlapping nodes and unnecessary connector crossings.",
  "- Keep labels short and place them inside nodes whenever possible.",
  "- Make every connector explicit between the referenced nodes.",
  "- If the image is messy, simplify rather than inventing extra detail.",
].join("\n");
...
function promptRulesForQualityPreset(qualityPreset) {
  return qualityPreset === "structured" ? STRUCTURED_RULES : BALANCED_RULES;
}

export function buildUserPrompt(userNotes, qualityPreset = "balanced") {
  const trimmedNotes =
    typeof userNotes === "string" && userNotes.trim() ? userNotes.trim() : "";
  const rulesBlock = promptRulesForQualityPreset(qualityPreset);
      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            {
              type: "text",
              text: buildUserPrompt(userNotes, qualityPreset),
            },
            {
              type: "image_url",
              image_url: {
